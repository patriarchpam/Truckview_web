import React from 'react'
import { Link } from 'react-router-dom'
import { PhoneIcon, MailIcon, MapPinIcon } from 'lucide-react'

export function Footer() {
  return (
    <footer className="bg-navy-950 text-navy-300">
      <div className="mx-auto max-w-7xl px-4 py-16">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {/* Brand */}
          <div>
            <Link to="/" className="flex items-center gap-3 group">
              <img src="/logo.png" alt="Truck-View" className="h-10 w-auto brightness-0 invert" />
              <div>
                <span className="block text-lg font-bold text-white leading-tight">Truck-View</span>
                <span className="block text-[10px] uppercase tracking-widest text-navy-400 font-medium">Global Ent.</span>
              </div>
            </Link>
            <p className="mt-4 text-sm leading-relaxed">
              Professional mechanic workshop serving Abuja and surrounding areas. Quality vehicle servicing made simple.
            </p>
          </div>

          {/* Quick links */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-white mb-4">Quick Links</h3>
            <ul className="space-y-2.5 text-sm">
              {[
                { to: '/services', label: 'Our Services' },
                { to: '/vehicle-types', label: 'Vehicle Types' },
                { to: '/pricing', label: 'Pricing Plans' },
                { to: '/book', label: 'Book a Service' },
                { to: '/booking', label: 'Track Booking' },
              ].map((l) => (
                <li key={l.to}>
                  <Link to={l.to} className="hover:text-accent-400 transition-colors">{l.label}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-white mb-4">Services</h3>
            <ul className="space-y-2.5 text-sm">
              {['Routine Maintenance', 'Vehicle Inspection', 'Engine Diagnostics', 'Tyres & Brakes', 'Detailing & Care', 'Mobile Assistance'].map((s) => (
                <li key={s}>
                  <Link to="/services" className="hover:text-accent-400 transition-colors">{s}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-white mb-4">Contact</h3>
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2.5">
                <MapPinIcon size={16} className="mt-0.5 shrink-0 text-accent-400" />
                Behind Games Village, Deck One Apartments, Karu District, Abuja
              </li>
              <li className="flex items-center gap-2.5">
                <PhoneIcon size={16} className="shrink-0 text-accent-400" />
                <a href="tel:08036798700" className="hover:text-accent-400 transition-colors">0803 679 8700</a>
              </li>
              <li className="flex items-center gap-2.5">
                <MailIcon size={16} className="shrink-0 text-accent-400" />
                <a href="mailto:truckviewent@gmail.com" className="hover:text-accent-400 transition-colors">truckviewent@gmail.com</a>
              </li>
            </ul>
            <div className="mt-5 flex gap-3">
              <a href="https://wa.me/2348036798700" target="_blank" rel="noreferrer"
                className="rounded-lg bg-navy-800 p-2.5 text-navy-300 hover:text-white hover:bg-navy-700 transition-colors">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492a.5.5 0 00.612.638l4.685-1.378A11.933 11.933 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22a9.94 9.94 0 01-5.382-1.572l-.376-.227-2.785.82.747-2.73-.25-.397A9.94 9.94 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
              </a>
              <a href="https://instagram.com" target="_blank" rel="noreferrer"
                className="rounded-lg bg-navy-800 p-2.5 text-navy-300 hover:text-white hover:bg-navy-700 transition-colors">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
              <a href="https://facebook.com" target="_blank" rel="noreferrer"
                className="rounded-lg bg-navy-800 p-2.5 text-navy-300 hover:text-white hover:bg-navy-700 transition-colors">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
            </div>
          </div>
        </div>

        <div className="mt-12 border-t border-navy-800 pt-8 text-center text-xs text-navy-500">
          © {new Date().getFullYear()} Truck-View Global Ent. All rights reserved.
        </div>
      </div>
    </footer>
  )
}
